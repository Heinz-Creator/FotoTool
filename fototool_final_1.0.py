...
class FotoToolFinal20(tk.Tk):
        ...
        def worker():
            try:
                ...
                summary = "\n".join([
                    ...
                    f"WhatsApp: {count_whatsapp}",
                    f"Screenshots: {count_screenshot}",
    ...
    def _update_structure_preview(self, *args):
        ...
        if mode == "exif":
            ...
            preview = (
                ...
                f"WhatsApp → Archiv → WhatsApp → {year} → {month}\n"
                f"Screenshots → Archiv → Screenshots → {year} → {month}"
            )
    ...
    def _build_event_name(self, event_id, has_real_gps, lat, lon, event_start_date, event_end_date, event_place_cache):
        """
        Erzeugt einen stabilen, menschenlesbaren Eventnamen.
        Unterstützt auch Mehr-Tage-Events.
        """
    ...
    def _run_sort(self):
        ...
        if getattr(self, "write_tags_var", None) and self.write_tags_var.get():
            ok = messagebox.askyesno(
                "Hinweis: Metadaten werden geschrieben",
                "Du hast aktiviert: Schlagwörter ins Bild schreiben.\n\n"
                "Dabei werden im Zielarchiv Schlagwörter dauerhaft im Bild gespeichert.\n"
                "Originale im Import bleiben bei 'Kopieren' unverändert.\n\n"
                "Fortfahren?"
            )
            if not ok:
                return
        ...
        def worker():
            try:
                ...
                for idx, file in enumerate(files, start=1):
                        ...
                        if target_dir is None and category == "event":
                            ...
                            if not has_real_gps:
                                ...
                                if not d0:
                                    ...
                                    if structure == "place_first":
                                        target_dir = os.path.join(self.archiv_var.get(), "Alltag", year, month_name)

                                    elif structure == "time_first":
                                        target_dir = os.path.join(self.archiv_var.get(), year, month_name, event_name)


                                    elif structure == "place_event":
                                        target_dir = os.path.join(self.archiv_var.get(), "Alltag", date_str)

                                    else:
                                        target_dir = os.path.join(self.archiv_var.get(), "Alltag", year, month_name)

                                    count_sorted += 1
                                    exif_flag = "Event (Alltag)"
                            ...
                            # -------- MIT GPS --------
                                ...
                                if structure == "time_first":
                                    ...
                                    target_dir = os.path.join(self.archiv_var.get(), place_dir, year, month_name)
                    ...
                    # --- Tags ins Ziel schreiben (nur wenn aktiviert) ---
                    if getattr(self, "write_tags_var", None) and self.write_tags_var.get():
                        tags = self.tags.tags_for_file(file)
                        if tags and not has_exif:
                            ok = self._write_tags_to_file(dest, tags)
                            if ok:
                                count_tags_written += 1
                                self._log_line(f"TAGWRITE: {dest} -> {tags}")
                            else:
                                self._log_line(f"TAGWRITE_FAIL: {dest} -> {tags}")
                ...
                lines.append("")
                lines.append(f"Tags geschrieben: {count_tags_written}")
    ...
    def _page_sort(self, parent):
        ...
        self.write_tags_var = tk.BooleanVar(value=False)
        ...
        tk.Checkbutton(
            mid_col,
            text="Schlagwörter ins Bild schreiben (dauerhaft gespeichert)",
